...
#обработка для голосовых запросов(потраченных)
def insert_row_tts(user_id, message, tts_symbols, db_name="speech_kit.db"):
    try:
        # Подключаемся к базе
        with sqlite3.connect(db_name) as conn:
            cursor = conn.cursor()
...
def insert_row_stt(user_id, message, stt_symbols, db_name="speech_kit.db"):
    try:
        # Подключаемся к базе
        with sqlite3.connect(db_name) as conn:
            cursor = conn.cursor()
            # Вставляем в таблицу новое сообщение
            cursor.execute('''INSERT INTO messages (user_id, message, stt_symbols)VALUES (?, ?, ?)''',
